        ...
        public void SpeedTypeData(string version, byte SpeedType)
        {
            if (speedNames.ContainsKey(version))
            {
                if (version == "国服")
                {
                    ...
                    else if (!speedNames[version].ContainsValue(SpeedType))
                    {
                        Default();
                    }
                }
